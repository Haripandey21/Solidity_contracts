...
describe("shipBlood function checking...", () => {
    beforeEach(async () => {
        [owner,addr1, addr2, addr3] = await hre.ethers.getSigners();
        instanceContract = await hre.ethers.getContractFactory("BloodSupply");
        deployedContract = await instanceContract.deploy();
        await deployedContract.addSupplier(addr1.address, "Luis blood suppliers",9016713333); 
    });

    it("Should ship the blood.. ", async () => {
        await deployedContract.addHospital(addr3.address, "Bir Hospital",9016713300);
